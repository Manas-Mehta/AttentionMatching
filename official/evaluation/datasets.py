...
def load_keylen_data(task: str) -> List[Dict]:
    """
    Load the key-length sweep dataset (the "UUID Bit" experiment).

    Reads ``data/keylen/{task}.jsonl`` where task is e.g. ``keylen_16`` / ``keylen_32``
    / ``keylen_64`` / ``keylen_96`` (bits). These rows are real RULER niah_single_3 4k
    documents with ONLY the key string swapped for an N-bit random-hex key, generated by
    ``experiments/make_keylen_data.py``. Same standardized format as ``load_ruler_data``.
    """
    local_dir = Path('data/keylen')
    fp = local_dir / f'{task}.jsonl'
    if not fp.exists():
        raise ValueError(
            f"Key-length dataset not found: {fp}. Generate it first with "
            f"`python experiments/make_keylen_data.py`."
        )
    rows = []
    with open(fp) as f:
        for line in f:
    ...
    # Group by unique context (each keylen doc has a unique context -> 1 article each),
    # mirroring load_ruler_data's standardized output shape.
    context_groups = {}
    for idx, row in enumerate(rows):
        ctx_key = hash(row['context'])
        if ctx_key not in context_groups:
            context_groups[ctx_key] = {
                'context': row['context'],
                'questions': [],
        ...
        context_groups[ctx_key]['questions'].append({
            ...
            'max_new_tokens': row.get('max_new_tokens', 128),
            'task': row['task'],
            'question_unique_id': f"{task}_{idx}",
        })

    data = []
    for ctx_key, group in context_groups.items():
        data.append({
            'article_id': f"{task}_{group['article_idx']}",
            'title': f"keylen {task} (article {group['article_idx']})",
            'article': group['context'],
            'questions': group['questions'],
        })
    print(f"Loaded {len(data)} keylen articles for {task}")
    return data
...
def load_dataset(dataset_name: str, include_diagnosis: bool = True) -> List[Dict]:
    ...
    elif dataset_name.startswith('keylen'):
